...
static void startNetwork() {
  ...
  if (!cfg.wifiSsid.isEmpty()) {
    ...
    if (cfg.wifiStaticIp) {
      ...
      const bool valid = ip.fromString(cfg.wifiIp) && gateway.fromString(cfg.wifiGateway) && subnet.fromString(cfg.wifiSubnet) && dns1.fromString(cfg.wifiDns1) && dns2.fromString(cfg.wifiDns2);
...
void setup() {
  ...
  ++rtcBootCount; data.bootCount = rtcBootCount;
  store.load(cfg);
  loadNesaConfig(cfg);
  startNetwork();
